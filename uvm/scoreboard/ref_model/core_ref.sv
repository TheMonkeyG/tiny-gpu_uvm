class core_ref;
    ...
    function void execute_block(
        ...
        while (instr_count < 1000) begin
            instr_count++;
            d = decoder_ref::decode(prog_mem[current_pc]);
            ...
            for (int t = 0; t < active_threads; t++) begin
                ...
                if (d.alu_output_mux) alu_out = {5'b0, alu_ref::compare(rs_val, rt_val)};
                else alu_out = alu_ref::compute(d.alu_arithmetic_mux, rs_val, rt_val);

                if (d.mem_read_enable) lsu_out = data_mem[rs_val];

                if (d.mem_write_enable) begin
                    mem_write_t mw;
                    data_mem[rs_val] = rt_val;
                    mw.addr = rs_val; mw.data = rt_val;
