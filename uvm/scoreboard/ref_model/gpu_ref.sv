class gpu_ref_model;
    ...
    function new();
        ...
        for (int i = 0; i < NUM_CORES; i++) cores[i] = new(THREADS_PER_BLOCK);
        reset();
    endfunction
    ...
    function void load_prog(logic [7:0] addr, logic [15:0] data); prog_mem[addr] = data; endfunction
    function void load_data(logic [7:0] addr, logic [7:0] data); data_mem[addr] = data; endfunction
    function void set_thread_count(int tc); thread_count = tc; endfunction

    function void execute();
        int total_blocks = dispatcher.get_total_blocks(thread_count);
        int blocks_dispatched = 0;
        
        for (int i = 0; i < 256; i++) expected_data_mem[i] = data_mem[i];
        expected_writes.delete();

        while (blocks_dispatched < total_blocks) begin
            for (int c = 0; c < NUM_CORES && blocks_dispatched < total_blocks; c++) begin
                int block_tc = dispatcher.get_block_thread_count(blocks_dispatched, thread_count);
                cores[c].execute_block(blocks_dispatched[7:0], block_tc, prog_mem, expected_data_mem);
                foreach (cores[c].mem_writes[w]) expected_writes.push_back(cores[c].mem_writes[w]);
                blocks_dispatched++;
            end
        end
        executed = 1;
